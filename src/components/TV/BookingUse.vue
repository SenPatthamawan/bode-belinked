<template>
        <div class="col d-flex flex-column">
          <div class="d-flex flex-row">
            <div class="col-6 d-flex flex-column justify-content-center align-items-center">
                ...
                <div class="col-6 d-flex flex-column justify-content-center align-items-center">
                    <div><span class="text-Header">Counter</span></div>
        ...
        <div v-for="(index) in 6" :key="index" :class="`d-flex flex-row row-service my-1 ${bookingUse && bookingUse.length > 0 ? bookingUse.length >= index ?  'service' : 'none-service' : 'none-service'}`" >
            <div class="col-6 d-flex justify-content-center align-items-center col-left py-0"><span class="text-service">{{bookingUse && bookingUse.length > 0  ? bookingUse.length >= index ? bookingUse[index-1].storeFrontQueue  : '': ''}}</span></div>
             <div class="col-6 d-flex justify-content-center align-items-center col-right py-0"><span class="text-service">{{bookingUse && bookingUse.length > 0  ? bookingUse.length >= index ? bookingUse[index-1].storeFrontQueue ? bookingUse[index-1].servicePoint ? bookingUse[index-1].servicePoint : textRunQueue : '' : '' : ''}}</span></div>
            </div>
        </div>
</template>
<script>
export default {
  props: {
    bookingUse: { type: Array, default: () => [] },
    isPortrait: {type: Boolean, default: false},
    textRunQueue: { type: String, default: 'เรียกคิว' }
  }
}
</script>
<style scoped>
