                      ...
                      <form>
                        <v-row>
                          <v-col cols="6">
                            <v-text-field
                              label="runningCode"
                              placeholder="runningCode"
                              v-model="search.runningCode"
                              outlined
                              dense
                            ></v-text-field>
                          </v-col>
                          <v-col cols="6">
                            <v-text-field
                              label="runningName"
                              placeholder="runningName"
                              v-model="search.runningName"
                              outlined
                              dense
                            ></v-text-field>
                          </v-col>
                          <v-col cols="4">
                            <v-text-field
                              label="runningYear"
                              placeholder="runningYear"
                              v-model="search.runningYear"
                              maxlength="4"
                              outlined
                              dense
                            ></v-text-field>
                          ...
                          <v-col cols="4">
                            <v-text-field
                              label="runningMonth"
                              placeholder="runningMonth"
                              v-model="search.runningMonth"
                              maxlength="2"
                              outlined
                              dense
                            ></v-text-field>
                          ...
                          <v-col cols="4">
                            <v-text-field
                              label="runningNumber"
                              placeholder="runningNumber"
                              v-model="search.runningNumber"
                              maxlength="10"
                              outlined
                              dense
                            ></v-text-field>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <v-text-field
                        label="runningCode*"
                        v-model="formUpdate.runningCode"
                        :rules="nameRules"
                        :counter="30"
                        maxlength="30"
                        required
                    ...
                    <v-col cols="12">
                      <v-text-field
                        label="runningName*"
                        v-model="formUpdate.runningName"
                        :rules="nameRules"
                        :counter="30"
                        maxlength="30"
                        required
                    ...
                    <v-col cols="12">
                      <v-text-field
                        label="runningYear*"
                        v-model="formUpdate.runningYear"
                        :rules="nameRules"
                        :counter="4"
                        maxlength="4"
                        required
                    ...
                    <v-col cols="12">
                      <v-text-field
                        label="runningMonth*"
                        v-model="formUpdate.runningMonth"
                        :rules="nameRules"
                        :counter="2"
                        maxlength="2"
                        required
                    ...
                    <v-col cols="12">
                      <v-text-field
                        label="runningNumber*"
                        v-model="formUpdate.runningNumber"
                        :rules="nameRules"
                        :counter="30"
                        maxlength="30"
                        required
...
export default {
  ...
  data () {
    return {
      PK: '',
      path: '/system_running/', // Path Model
      // Menu Config
      breadcrumbs: [
        {
          text: 'Home',
      ...
      // Data Table Config
      columns: [
        { text: 'runningID', value: 'runningID' },
        { text: 'runningCode', value: 'runningCode' },
        { text: 'runningName', value: 'runningName' },
        { text: 'runningYear', value: 'runningYear' },
        { text: 'runningMonth', value: 'runningMonth' },
        { text: 'runningNumber', value: 'runningNumber' },
      ...
      // Export Config
      exportType: 'xls',
      exportFileName: 'System-Running.xls',
  ...
  methods: {
    async getDataById (item) {
      ...
      // Get ID /main.js
      this.dataReady = false
      this.getDataByIdGlobal(this.DNS_IP, this.path, 'runningID', item.runningID)
    },
    async addData () {
      //
      //
      ...
      //
      this.formAdd.runningCode = this.code + this.generateCodeGlobal()
      //
      // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // //
    ...
    async searchDataAll () {
      ...
      //
      var search =
        '?runningCode=' + this.searchAll + '&runningName=' + this.searchAll + '&runningYear=' + this.searchAll + '&runningMonth=' + this.searchAll + '&runningNumber=' + this.searchAll
      this.dataReady = false
      ...
      this.$swal({
        ...
        .then(async (result) => {
          for (var key in this.dataItemImport) {
            this.PK = this.dataItemImport[key].runningID
            if (action === 'add') {
              console.log('add')
              ...
              await axios
                .get(
                  ...
                  this.DNS_IP +
                    this.path +
                    'get?runningID=' +
                    this.dataItemImport[key].runningID,
                  {
                    headers: {
                ...
                .catch((error) => {
                  ...
                  console.log('error /master_application/get?runningID : ', error)
                })
            }
          }
    ...
    async importDataAdd (dt) {
      ...
      await axios
        ...
        .then(async (response) => {
          ...
          // Debug response
          console.log('/master_application/add/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
    ...
    async importDataUpdate (dt) {
      ...
      delete this.formUpdate['runningID']
      ...
      await axios
        .post(
          // eslint-disable-next-line quotes
          this.DNS_IP + this.path + "edit/" + dt.runningID,
          this.formUpdateItem,
          {
            headers: {
              'Application-Key': this.$session.getAll().ApplicationKey
        ...
        .then(async (response) => {
          this.dialogImport = false
          // Debug response
          console.log('/master_application/edit/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
          alert('โปรดใส่ไฟล์ให้ถูกต้อง', dt)
    ...
    async importDataDelete (dt) {
      ...
      await axios
        .post(
          // eslint-disable-next-line quotes
          this.DNS_IP + this.path + "delete/" + dt.runningID,
          this.formUpdate,
          {
            headers: {
              'Application-Key': this.$session.getAll().ApplicationKey
        ...
        .then(async (response) => {
          this.dialogImport = false
          // Debug response
          console.log('/master_application/delete/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
          alert('โปรดใส่ไฟล์ให้ถูกต้อง', dt)
