                    ...
                    <v-card-text>
                      <form>
                        <v-row>
                          <v-col cols="6">
                            <v-text-field
                              label="privacyConfigCode"
                              placeholder="privacyConfigCode"
                              v-model="search.privacyConfigCode"
                              outlined
                              dense
                            ></v-text-field>
                          </v-col>
                          <v-col
                            ...
                            >
                              <v-select
                                v-model="search.privacyConfigStatus"
                                :items="statusUse"
                                label="สถานะใช้งาน"
                                outlined
                                dense
            ...
            <v-card>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <vue-editor v-model="formAdd.privacyConfigDetail"></vue-editor>
                      <!-- <v-text-field
                        label="privacyConfigDetail*"
                        v-model="formAdd.privacyConfigDetail"
                        :rules="nameRules"
                    ...
                    >
                      <v-select
                        v-model="formAdd.privacyConfigStatus"
                        :items="statusUse"
                        label="สถานะใช้งาน"
                        outlined
                        dense
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <v-text-field
                        label="privacyConfigCode*"
                        v-model="formUpdate.privacyConfigCode"
                        :rules="nameRules"
                    ...
                    <v-col cols="12">
                       <vue-editor v-model="formUpdate.privacyConfigDetail" ></vue-editor>
                      <!-- <v-text-field
                        label="privacyConfigDetail*"
                        v-model="formUpdate.privacyConfigDetail"
                        :rules="nameRules"
                    ...
                    >
                      <v-select
                        v-model="formUpdate.privacyConfigStatus"
                        :items="statusUse"
                        label="สถานะใช้งาน"
                        outlined
                        dense
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12">
                    <vue-editor v-model="formUpdate.privacyConfigDetail" disabled></vue-editor>
                    </v-col>
                    <v-col
                      cols="3"
                    >
...
export default {
  ...
  data () {
    return {
      ...
      path: '/system_privacy/', // Path Model
      // Menu Config
      breadcrumbs: [
        {
          text: 'Home',
      ...
      columns: [
        ...
        { text: 'privacyConfigCode', value: 'privacyConfigCode' },
        { text: 'privacyConfigStatus', value: 'privacyConfigStatus' },
      ...
      columnsImport: [
        { text: 'privacyConfigId', value: 'privacyConfigId' },
        { text: 'privacyConfigCode', value: 'privacyConfigCode' },
        { text: 'privacyConfigDetail', value: 'privacyConfigDetail' },
        { text: 'privacyConfigStatus', value: 'privacyConfigStatus' }
      ],
      ...
      // Export Config
      exportType: 'xls',
      exportFileName: 'Master-Privacy.xls',
      exportWorksheet: 'WorkSheet',
      export_fields: {
        privacyConfigId: 'privacyConfigId',
        privacyConfigCode: 'privacyConfigCode',
        privacyConfigDetail: 'privacyConfigDetail',
  ...
  methods: {
    async getDataById (item) {
      ...
      // Get ID /main.js
      this.dataReady = false
      this.getDataByIdGlobal(this.DNS_IP, this.path, 'privacyConfigId', item.privacyConfigId)
    },
    async addData () {
      //
      //
      ...
      //
      this.formAdd.privacyConfigCode = this.code + this.generateCodeGlobal()
      //
      // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // //

      await this.updateStatusoff(this.dataItem)

      this.dataReady = false
      await this.addDataGlobal(this.DNS_IP, this.path, this.formAdd)
    },
    async editData () {
      ...
      // ปรับให้เป็น OFF ก่อน
      await this.updateStatusoff(this.dataItem)

      this.dataReady = false
      await this.editDataGlobal(this.DNS_IP, this.path, this.PK, this.formUpdateItem)
    },
    async updateStatusoff (dt) {
      console.log('dataItem ', dt)
      var s = {}
      s.privacyConfigStatus = 'OFF'
      for (var rs1 in dt) {
        console.log(dt[rs1].privacyConfigId)
        await axios
          .post(
          // eslint-disable-next-line quotes
            this.DNS_IP + this.path + "edit/" + dt[rs1].privacyConfigId,
            s,
            {
              headers: {
    ...
    async searchDataAll () {
      ...
      //
      var search =
        '?privacyConfigCode=' + this.searchAll + '&privacyConfigDetail=' + this.searchAll
      this.dataReady = false
      ...
      this.$swal({
        ...
        .then(async (result) => {
          for (var key in this.dataItemImport) {
            this.PK = this.dataItemImport[key].privacyConfigId
            if (action === 'add') {
              console.log('add')
              ...
              await axios
                .get(
                  ...
                  this.DNS_IP +
                    this.path +
                    'get?privacyConfigId=' +
                    this.dataItemImport[key].privacyConfigId,
                  {
                    headers: {
                      'Application-Key': this.$session.getAll().ApplicationKey
                    }
                ...
                .catch((error) => {
                  ...
                  console.log('error /system_privacy/get?privacyConfigId : ', error)
                })
            }
          }
    ...
    async importDataAdd (dt) {
      ...
      delete this.formAdd['privacyConfigId']
      ...
      await axios
        ...
        .then(async (response) => {
          ...
          // Debug response
          console.log('/system_privacy/add/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
    ...
    async importDataUpdate (dt) {
      ...
      delete this.formUpdate['privacyConfigId']
      ...
      await axios
        .post(
          // eslint-disable-next-line quotes
          this.DNS_IP + this.path + "edit/" + dt.privacyConfigId,
          this.formUpdateItem,
          {
            headers: {
              'Application-Key': this.$session.getAll().ApplicationKey
        ...
        .then(async (response) => {
          this.dialogImport = false
          // Debug response
          console.log('/system_privacy/edit/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
          alert('โปรดใส่ไฟล์ให้ถูกต้อง', dt)
    ...
    async importDataDelete (dt) {
      ...
      await axios
        .post(
          // eslint-disable-next-line quotes
          this.DNS_IP + this.path + "delete/" + dt.privacyConfigId,
          this.formUpdate,
          {
            headers: {
              'Application-Key': this.$session.getAll().ApplicationKey
        ...
        .then(async (response) => {
          this.dialogImport = false
          // Debug response
          console.log('/system_privacy/delete/', response)
        })
        // eslint-disable-next-line handle-callback-err
        .catch((error) => {
          alert('โปรดใส่ไฟล์ให้ถูกต้อง', dt)
